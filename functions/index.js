// Importa las funciones de la v2 (HTTPS onCall)
const { https } = require("firebase-functions/v2");
const axios = require("axios");
const admin = require("firebase-admin");

// Inicializar Firebase Admin SDK inmediatamente
admin.initializeApp();
 ...
 */
exports.getLiveQuote = https.onCall(async (request) => {
  ...
  try {
    ...
    if (error.response) {
        console.error("Alpha Vantage API Response Error:", error.response.data);
 ...
 */
exports.searchSymbol = https.onCall(async (request) => { // CAMBIADO a 'request' para consistencia en v2
  // Asegúrate de que la clave API esté definida para esta invocación
  if (!ALPHA_VANTAGE_API_KEY) {
  ...
  const { keywords } = request.data; // Acceso a los datos con request.data
  ...
  try {
    ...
    if (bestMatches && Array.isArray(bestMatches) && bestMatches.length > 0) { // Añadida verificación Array.isArray
