...
import ProfileModal from './ProfileModal'; // Asumo que este componente existe
import avatars from '../utils/avatars'; // Asumo que este util existe

// --- Iconos SVG para el Layout (mejor encapsulados) ---
...
function Layout() {
    ...
    const { userConfig, updateConfig } = useFinanceData();
    ...
    const currentAvatar = avatars.find(a => a.id === userConfig?.profile?.avatarId) || avatars[0];
    ...
    const sidebarContent = (
        <div className="flex flex-col h-full">
            ...
            {/* La sección de información de usuario y logout se ha eliminado de aquí */}
        </div>
    );
    ...
    return (
        <div className="flex min-h-screen bg-gray-900 text-gray-100">
            ...
            <aside className={`fixed inset-y-0 left-0 w-64 bg-gray-800 p-4 shadow-lg z-40 transform transition-transform duration-300 ease-in-out md:hidden ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                {sidebarContent}
            </aside>

            {/* Sidebar para escritorio */}
            <aside className={`hidden md:flex flex-col bg-gray-800 shadow-lg transition-all duration-300 ease-in-out ${isDesktopSidebarCollapsed ? 'w-24 p-2' : 'w-64 p-4'}`}>
                {sidebarContent}
            </aside>
            ...
            <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-auto">
                <header className="flex justify-between items-center mb-6">
                    ...
                    <button
                        ...
                        className="p-2 hidden md:block rounded-full bg-gray-700/50 text-gray-300 hover:bg-gray-700 focus:outline-none"
                    ...
                    <button
                        ...
                        className="p-2 md:hidden rounded-full bg-gray-700/50 text-gray-300 hover:bg-gray-700 focus:outline-none"
            ...
            {/* --- CAMBIO CLAVE: Pasar la función de logout al modal --- */}
            <ProfileModal
