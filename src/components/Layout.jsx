...
const navLinks = [
    ...
    { to: "/monthly-tracker", text: "Seguimiento Mensual", icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg> },
    { to: "/net-worth", text: "Patrimonio Neto", icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg> },
    { to: "/amortization", text: "Prestamos", icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg> },
    ...
    { to: "/ai-analysis", text: "Análisis IA", icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> },
...
function Layout() {
    ...
    // MODIFICACIÓN: Lógica para determinar la URL del avatar
    const getAvatarSrc = useCallback(() => {
        if (currentUser && !loadingData && userConfig) {
            // Prioriza la URL de avatar personalizada si existe
            if (userConfig.profile?.avatarUrl) {
                return userConfig.profile.avatarUrl;
            }
            // Si no hay URL personalizada, usa el avatarId para buscar en la lista predefinida
            const foundAvatar = avatars.find(a => a.id === userConfig.profile?.avatarId);
            if (foundAvatar) {
                return foundAvatar.url;
            }
        }
        // Si no hay usuario, cargando, o no se encuentra avatar, usa el primer avatar por defecto
        return avatars[0].url;
    }, [currentUser, loadingData, userConfig]);
    ...
    return (
        <div className="flex min-h-screen bg-gray-900 text-gray-100 dark:bg-gray-900 dark:text-gray-100">
            ...
            {/* Se ha eliminado la clase 'overflow-auto' para evitar la doble barra de scroll */}
            <main className="flex-1 p-4 sm:p-6 lg:p-8">
                <header className="flex justify-between items-center mb-6">
                    ...
                    {currentUser && !loadingData && userConfig && (
                        <img
                            // MODIFICACIÓN: Usar getAvatarSrc para la URL
                            src={getAvatarSrc()}
